...
- **VideoEmbed**: a click-to-play facade — nothing reaches youtube.com until someone asks for the video, then a `youtube-nocookie.com` iframe replaces the still. Two consequences worth keeping. **The poster is a local asset, never YouTube's thumbnail**: `i.ytimg.com` is a YouTube domain, so hotlinking one would call YouTube on load and break the whole point (it would also drag in the video's magenta-and-gold conference title card, on a page where gold only hints). **The play disc is white at rest and brand red on hover**, because the poster has a red wall in it and a red disc half-disappears into it. `ratio` sets the slot's aspect — landscape by default, `'9 / 16'` for a phone-shot reel — and `youtubeId={null}` still renders the pending slot, so pulling the video degrades to a link rather than an empty box.
...
## Copy rules
No emojis. Numbers over adjectives. Pending content is shown as pending (refund policy) — never filled in. Neither video in the original page outline was Chef Deb's (a Mel Robbins clip and a reel from another account), so neither was ever embedded; the band now plays her Global Culinary Conference masterclass, credited to that conference rather than dressed up as SHARPEN footage. Social links are the ones in chefdeb.com's own header. Internal discount codes never appear in the build; only EARLYBIRD and BAC are public.
