...
## Product Purpose
A single landing page that sells seats to SHARPEN 2027, the 11th annual two-day strategic-planning workshop run by Chef Deb Coaching (Thurs. Jan. 28 – Fri. Jan. 29, 2027, Meacham International Airport Conference Center, Fort Worth, TX). Success = ticket purchases through the GoHighLevel checkout (priority #1 in the brief), with a flawless mobile experience (priority #2). Secondary goals: sponsor enquiries and agenda PDF downloads.
...
**The Sept 2026 steers, in order.** First the client's note that *"all business owners want to know is how much money will it make me and how fast"* — which put the Power of One thesis in the hero (the money is in price, cost and volume, all three of which the owner already controls) with a 90-day plan as the answer to "how fast". Then, later the same month and marked ASAP, the client supplied replacement hero copy verbatim: *"Build a business that grows on purpose, not by accident."* plus steady growth, repeat clients, a business that works for the owner's life, closing on **From the Stove to CEO™**. That is what the hero says now. The meta description and llms.txt were realigned to match, so the snippet, the brief and the first thing on the page now say one thing. The money framing did not disappear — it still carries the Profit track in the agenda, and llms.txt still names price, cost and volume as the levers — it simply stopped leading. No earnings figure is promised anywhere, because none was ever supplied; what is promised is that they run their own numbers in the room. Any future ROI copy inherits that constraint.

## Operating Context
- Checkout is off-site, on the client's GoHighLevel funnel domain: `c.chefdeb.com/sharpen-2027-early-bird`, `-regular` and `-bring-a-colleague`. It replaced Thrivecart in Sept 2026 so that a purchase creates a CRM contact. See [docs/ghl-checkout.md](docs/ghl-checkout.md).
...
- **There is no public discount code.** `EARLYBIRD` was retired in Sept 2026 with the `checkoutUrl()` coupon helper, and `BAC` (second ticket half off) became a real two-seat product at $592.50 rather than a code — GoHighLevel coupons discount the whole order, so a 50% code would have halved both tickets.
...
- The client holds several internal comp/scholarship discount codes (kept in the ClickUp task, not in this repo). They must never appear in the build or in this repository. No public code exists.
